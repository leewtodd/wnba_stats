...
## DEC-008: Registry-Based Auto-Discovery for Extensibility
- **Date:** 2026-03-25
- **Phase:** Architecture
- **Decision:** The system uses auto-discovery patterns at all three extension points: data sources (subclass discovery in sources/), analysis modules (decorator-based function registry in engine/), and viz types (function-per-chart). New capabilities are added by creating files — no config files, no wiring code, no modifications to existing modules.
- **Alternatives Considered:**
  - Explicit registration in a config file: More visible but creates a maintenance burden and a single point of failure (forget to update config = silent breakage).
  - Plugin architecture with entry points: Over-engineered for a single-user local tool.
- **Rationale:** The system needs to support predictive modeling, social media sentiment, and unknown future features without rearchitecting. Auto-discovery means "drop a file and it works." The function registry also drives NLP tool generation — when a new engine function is registered, Claude's API automatically gets access to it as a tool without updating the system prompt manually.
- **Revisit When:** If auto-discovery creates ambiguity (e.g., a half-finished module gets picked up accidentally). At that point, add an explicit enable/disable mechanism.
- **Adversarial Review Status:** Warning raised on accidental discovery of incomplete modules. Mitigated: the decorator pattern means only explicitly decorated functions are registered. An undecorated function is invisible to the system.
